...
Schedule LaucScheduler::search( double schedTime, double schedDur, int &count ) {
	...
	*/	
	   ...
	   //#BEGIN NP-MOC, Header drop tail (LAUC)
	   ...
	   for( u_int i = ncc_; i < maxChannels_; i++ )
	   {           		   
		   if( schedTime >= unschTime_[i] )  // try to schedule after the void Lap lich ngoai khoang trong  LAUC
		   if( ( schedTime - unschTime_[i] ) < tempGap ) //
		   {
