/* NPMOCVF */
/* LAUCVF -- LAUC -- NPMOCVF (Drop Head, Tail, Head & Tail) -- NPMOC 
 *
 */
...
Schedule LaucScheduler::search( double schedTime, double schedDur, int &count ) {
	...
	while(1){	
	   ...
	   {		   
		...
		for( u_int i = ncc_; i < maxChannels_; i++ )
			if(startTime_[i]>=schedTime)
			if(endTime_[i]>=(schedTime + schedDur))
			if((startTime_[i] - schedTime)<tempOverlap)	//		
			{
				tempOverlap = startTime_[i]-schedTime;//Drop Header			
				result.channel() = i;
				result.startTime() = schedTime;
			}
		if (result.channel() >= 0)
