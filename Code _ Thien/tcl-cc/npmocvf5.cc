/* NPMOCVF */
/* LAUCVF -- LAUC -- NPMOCVF (Drop Head & Tail) -- NPMOC 
 *
 */
...
Schedule LaucScheduler::search( double schedTime, double schedDur, int &count ) {
	...
	while(1){	
	   ...
	   {		   
		...
		//NPMOC-VF, Drop Header
		tempOverlap = HUGE_VAL;
		for( u_int i = ncc_; i < maxChannels_; i++ )
			if((startTime_[i]>=schedTime) && ((schedTime + schedDur)>=endTime_[i]))
			if(((schedTime - startTime_[i]) + ((schedTime + schedDur) - endTime_[i]) )<tempOverlap)	//		
			{
				tempOverlap = (schedTime - startTime_[i]) + ((schedTime + schedDur) - endTime_[i]);//Drop Header & Tail			
				result.channel() = i;				
			}
		if (result.channel() >= 0)
		{
			schedTime=startTime_[result.channel()];
